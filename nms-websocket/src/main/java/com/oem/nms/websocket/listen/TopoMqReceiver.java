...
public class TopoMqReceiver {
    ...
    @RabbitHandler
    @RabbitListener(queues = MqConstants.MQ_QUEUE_WEBSOCKET_TOPO)
    public void handle(Message message, Channel channel) {
        Response<WebSocketMessage> response = MqUtil.handle(message, channel);
        if (response.isError()) {
            log.warn("Error in handle mq topo: {}", response.getMessage());
            return;
        }

        messagingTemplate.convertAndSend(WebsocketConstants.WEBSOCKET_TOPIC_TOPO, response.getData());
    }
